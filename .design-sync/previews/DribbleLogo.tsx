import { DribbleLogo } from "untitledui-vite-starter-kit";

/** The brand-coloured mark, as used on a social sign-in button. */
export const Default = () => <DribbleLogo colorful className="size-10" />;

/** Without `colorful` the glyph is drawn in `currentColor`, so it adopts the surrounding text colour. */
export const Colorful = () => (
    <div className="flex items-center gap-8">
        <DribbleLogo colorful className="size-8" />
        <DribbleLogo className="size-8 text-fg-primary" />
        <DribbleLogo className="size-8 text-fg-quaternary" />
    </div>
);

export const Sizes = () => (
    <div className="flex flex-wrap items-end gap-6">
        <DribbleLogo colorful className="size-5" />
        <DribbleLogo colorful className="size-6" />
        <DribbleLogo colorful className="size-8" />
        <DribbleLogo colorful className="size-12" />
    </div>
);
