// forked from design-sync lib/bundle.mjs - tsconfig-paths must not resolve an alias to a directory
        ...
        for (const r of rules) {
          ...
          for (const t of r.targets) {
            ...
            for (const ext of exts) {
              const cand = stem + ext;
              // A bare stem that names a directory must fall through to the
              // /index.* candidates below - esbuild cannot read a directory.
