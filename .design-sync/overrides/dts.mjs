// forked from design-sync lib/dts.mjs - resolve <Name>Props from the file that
// declares the component before falling back to any file of that name
...
export function propsBodyFor(name, ctx) {
  ...
  // the live one.
  let decl = null;
  // Several modules in one package can declare the same `<Name>Props` name -
  // this repo has three `InputProps` and three `ButtonProps`, and a plain
  // first-match scan resolves in filename order, handing Input the date
  // field's props. Search the file(s) that actually declare the component
  // first; only then fall back to the rest of the package.
  ...
  const ordered = ownPaths.size
    ? [...all.filter((f) => ownPaths.has(f.getFilePath())), ...all.filter((f) => !ownPaths.has(f.getFilePath()))]
    : all;
