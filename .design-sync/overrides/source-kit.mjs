// forked from design-sync lib/source-kit.mjs - adds cfg.srcExclude so the synth
// entry can skip files whose exports collide with another file's
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // Files kept OUT of the synth entry. The entry star-exports every source
    // file, and ESM drops any name that two star-exported modules both export -
    // which silently removed AppStoreButton, Modal, Dialog, ModalOverlay and
    // MastercardIcon from window.UntitledUI. Each file below duplicates export
    // names owned by another file; it is re-exported under explicit, aliased
    // names from .design-sync/alias-exports.ts (wired via cfg.extraEntries), so
    // nothing is lost. Discovery still walks these files, so their components
    // keep their cards.
    const SRC_EXCLUDE = [
      'src/components/base/buttons/app-store-buttons-outline.tsx',
      'src/components/application/slideout-menus/slideout-menu.tsx',
      'src/components/shared-assets/credit-card/icons.tsx',
    ];
    const excluded = (p) => SRC_EXCLUDE.some((e) => slash(p).endsWith(e));
    const comps = srcFiles.filter((p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p) && !excluded(p));
    console.error(`  [SRC_EXCLUDE] ${srcFiles.filter(excluded).length} file(s) kept out of the synth entry (re-exported via extraEntries)`);
